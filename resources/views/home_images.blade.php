								...
								<div class="col-md-12">
									<div class="card">
										<div class="card-header">
											<a href="{{route('homeimageCreate')}}" class="btn btn-link float-right">Create New Image </a>
										</div>
										<div class="card-body">
												...
												<table class="table table-head-bg-primary table-striped table-hover">
                                                    ...
                                                    <tbody>
														@foreach($home_images as $home_image)
															<tr>
																<td>{{ $loop->index + 1 }}</td>
																<td><img src="{{ $home_image->url }}" alt="home_image Image" style="max-width: 100px; height: 60px;"></td>
																<td>{{ $home_image->headertext }}</td>
																<td>{{ $home_image->bodytext }}</td>
																<td>{{ $home_image->created_at->diffForHumans(); }}</td>
																<td  class="td-actions text-right">
																	<div class="form-button-action">
                                                                        <a href="{{ route('home_image.edit', $home_image->id) }}" data-toggle="tooltip" title="Edit Image" class="btn btn-link <btn-simple-primary">
																			<i class="la la-edit"></i>
																		</a>
																		<form action="{{ route('home_image.destroy', $home_image->id) }}" method="POST" style="display: inline;">
																			@csrf
																			@method('DELETE')
																			<button type="submit" data-toggle="tooltip" title="Remove" onclick="return confirm('Are you sure you want to delete this image?')" class="btn btn-link btn-simple-danger">
