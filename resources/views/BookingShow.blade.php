							...
							<div class="col-md-12">
								<div class="card">
									<div class="card-header">
										<div class="card-title">Message <b>{{$booking->firstName}}</b></div>
									</div>
                                    <form method="POST" action="{{ route('booking.email', $booking->id) }}" enctype="multipart/form-data">
                                        @csrf
                                        <div class="card-body">
                                            <div class="form-group">
                                                <label for="name">First Name</label>
                                                <input type="text" disabled value="{{ $booking->firstName }}" class="form-control @error('firstName') is-invalid @enderror" name="firstName" value="{{ old('firstName') }}" id="firstName" required autocomplete="firstName" autofocus >
                                            </div>

                                            <div class="form-group">
                                                <label for="name">Last Name</label>
                                                <input type="text" disabled value="{{ $booking->lastName }}" class="form-control @error('lastName') is-invalid @enderror" name="lastName" value="{{ old('lastName') }}" id="lastName" required autocomplete="lastName" autofocus >
                                            </div>

                                            <div class="form-group">
                                                <label for="email">User Email</label>
                                                <input type="email" disabled value="{{ $booking->email }}" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" id="email" required autocomplete="email" autofocus >
                                            </div>

                                            <div class="form-group">
                                                <label for="email">Phone</label>
                                                <input type="text" disabled value="{{ $booking->phone }}" class="form-control @error('phone') is-invalid @enderror" name="phone" value="{{ old('phone') }}" id="phone" required autocomplete="phone" autofocus >
                                            </div>

                                            <div class="form-group">
                                                <label for="email">Date</label>
                                                <input type="text" disabled value="{{ $booking->date->diffForHumans();  }}" class="form-control @error('date->diffForHumans(); ') is-invalid @enderror" name="date->diffForHumans(); " value="{{ old('date->diffForHumans(); ') }}" id="date->diffForHumans(); " required autocomplete="date->diffForHumans(); " autofocus >
                                            </div>

                                            <div class="form-group">
                                                <label for="email">Description</label>
                                                <textarea  class="form-control @error('description') is-invalid @enderror" name="description" value="{{ old('description') }}" rows="5">{{$booking->description}}</textarea>
                                            </div>
                                        </div>
