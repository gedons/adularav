								...
								<div class="col-md-12">
									<div class="card">
										<div class="card-header">
											<a href="{{route('galleryCreate')}}" class="btn btn-link float-right">Create New Image </a>
										</div>
										<div class="card-body">
												...
												<table class="table table-head-bg-primary table-striped table-hover">
                                                    ...
                                                    <tbody>
														@foreach($galleries as $gallery)
															<tr>
																<td>{{ $loop->index + 1 }}</td>
																<td><img src="{{ $gallery->url }}" alt="gallery Image" style="max-width: 100px; height: 60px;"></td>
																<td>{{ $gallery->gallerytext }}</td>
																<td>{{ $gallery->created_at->diffForHumans(); }}</td>
																<td>
                                                                    <div class="form-button-action">
                                                                    <a href="{{ route('gallery.edit', $gallery->id) }}" data-toggle="tooltip" title="Edit Post" class="btn btn-link <btn-simple-primary">
																	...
																	<div class="form-button-action">
																		<form action="{{ route('gallery.destroy', $gallery->id) }}" method="POST" style="display: inline;">
																			@csrf
																			@method('DELETE')
																			<button type="submit" data-toggle="tooltip" title="Remove" onclick="return confirm('Are you sure you want to delete this slider?')" class="btn btn-link btn-simple-danger">
