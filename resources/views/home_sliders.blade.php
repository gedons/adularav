								...
								<div class="col-md-12">
									<div class="card">
										<div class="card-header">
											<a href="{{route('homesliderCreate')}}" class="btn btn-link float-right">Create New Slider </a>
										</div>
										<div class="card-body">
												...
												<table class="table table-head-bg-primary table-striped table-hover">
                                                    ...
                                                    <tbody>
														@foreach($home_sliders as $home_slider)
															<tr>
																<td>{{ $loop->index + 1 }}</td>
																<td><img src="{{ $home_slider->url }}" alt="home_slider Image" style="max-width: 100px; height: 60px;"></td>
																<td>{{ $home_slider->title }}</td>
																<td>{{ Str::limit($home_slider->description, 100) }}</td>
																<td>{{ $home_slider->created_at->diffForHumans(); }}</td>
																<td  class="td-actions text-right">
																	<div class="form-button-action">
                                                                        <a href="{{ route('home_slider.edit', $home_slider->id) }}" data-toggle="tooltip" title="Edit" class="btn btn-link <btn-simple-primary">
																			<i class="la la-edit"></i>
																		</a>                                                                 
																		<form action="{{ route('home_slider.destroy', $home_slider->id) }}" method="POST" style="display: inline;">
																			@csrf
																			@method('DELETE')
