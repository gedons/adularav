								...
								<div class="col-md-12">
									<div class="card">
										<div class="card-header">
											<a href="{{route('tourCreate')}}" class="btn btn-link float-right"> New Video Tour </a>
										</div>
										<div class="card-body">
												...
												<table class="table table-head-bg-primary table-striped table-hover">
                                                    ...
                                                    <tbody>
														@foreach($tours as $tour)
															<tr>
																<td>{{ $loop->index + 1 }}</td>
                                                                <td>{{ $tour->title }} @if ($tour->title) <b>minutes</b> @endif</td>
																<td>
                                                                    ...
                                                                    @if ($tour->url)
                                                                        <p>  <a href="{{ asset( $tour->url) }}" target="_blank">Open Video</a></p>
                                                                    @endif
                                                                </td>
																<td>{{ $tour->created_at->diffForHumans(); }}</td>
																<td>
																	<div class="form-button-action">
																		<a href="{{ route('tour.edit', $tour->id) }}" data-toggle="tooltip" title="Edit tour" class="btn btn-link <btn-simple-primary">
																			<i class="la la-edit"></i>
																		</a>
																		<form action="{{ route('tour.destroy', $tour->id) }}" method="POST" style="display: inline;">
																			@csrf
																			@method('DELETE')
																			<button type="submit" data-toggle="tooltip" title="Remove" onclick="return confirm('Are you sure you want to delete this video?')" class="btn btn-link btn-simple-danger">
